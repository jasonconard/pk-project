...
export class EventService {
  ...
  public static checkEvents(mapHelper: MapHelper, playerMesh: THREE.Mesh, pressedCodes: KeyCode[]): SceneEvent {
      ...
      if(chunk.visible) {
        chunk.buildings.forEach(building => {
          building.events.forEach(evt => {
            const dirCond = EventService.checkDirection(evt.conditions.direction, mapHelper);
            if(!dirCond) { return; }
            ...
            eventsHelper.push({ evt, buildingMesh: mapHelper.meshes[buildingKey] });
          });
        });
      }
    });
  ...
  private static checkDirection(dir: PlayerDirection, mh: MapHelper): boolean {
    if(!PlayerDirection[dir]) { return true; }
    return dir === mh.scenePlayer.dir;
  }
